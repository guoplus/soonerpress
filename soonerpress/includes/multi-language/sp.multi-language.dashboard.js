...
$(document).ready( function() {
	...
	$('#sp-pe-languages-selector a').click( function(e) {
		e.preventDefault();
		lang = $(this).data('lang');
		$('#sp-pe-languages-selector a')
			.removeClass('current')
	...
	$('#sp-pe-languages-selector .sp-ml-lang-tab-'+current_lang).trigger('click');
	...
	if ( $('form#addtag').length ) {
		...
		$('form#addtag').before($('.sp-te-languages-selector'));
	}

	if ( $('form#edittag').length ) {
		$('#name').after($('.sp-te-one-field-name-l'));
		$('#description').after($('.sp-te-one-field-description-l'));
		$('form#edittag .form-table:first > tbody').prepend('<tr><td colspan="2"></td></tr>').find('td:first').append($('.sp-te-languages-selector'));
	}

	$('.sp-te-languages-selector a').click( function(e) {
		e.preventDefault();
		lang = $(this).data('lang');
		$('.sp-te-languages-selector a')
			.removeClass('current')
			.filter('.sp-ml-lang-tab-'+lang).addClass('current');
		if ( main_stored_language == lang ) {
	...
	$('.sp-te-languages-selector .sp-ml-lang-tab-'+current_lang).trigger('click');
