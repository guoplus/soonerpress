...
function sp_post_meta_do_entry_html_field( $entry, $post_id, $lang = '' ) {
	...
	switch( $entry['type'] ) {
		...
		case 'group':
			// ==================== multiple group ====================
			$fields = $entry['fields'];
			$title_single = $entry['title_single'];
			if ( is_array( $value ) && $value )
				foreach ( $value as $k => $v ) {
					echo '<span class="sp-pm-multi-one-group">';
					printf( '<span class="sp-pm-multi-one-singletitle">%s</span>', $title_single );
					foreach( $fields as $f ) {
						sp_post_meta_do_entry_html_field_nested( $f, $id . '['.$k.']', $v );
					}
					echo '</span>';
				}
			echo '<span class="sp-pm-multi-one-def">';
			printf( '<span class="sp-pm-multi-one-singletitle">%s</span>', $title_single );
			foreach( $fields as $f ) {
...
function sp_post_meta_do_entry_html_field_nested( $entry, $id_parent, $value_origin = '' ) {
	...
	$multiple = isset( $entry['multiple'] ) && $entry['multiple'];
	$value = ( ! empty( $value_origin ) ? $value_origin[$entry['id']] : '' );
	$std = isset( $entry['std'] ) ? $entry['std'] : '';
					...
					foreach( $box['fields'] as $f ) {
						if ( isset( $_POST[$f['id']] ) ) {
							...
							if ( sp_in_ml() && ( ( ! isset( $f['ml'] ) ) || ( isset( $f['ml'] ) && $f['ml'] ) ) ) {
								$data['ml'] = true;
								// $data = sp_serialize( $data );
							} else {
								// if ( $f['multiple'] || 'group' == $f['type'] )
								// 	$data = sp_serialize( $data );
							}
